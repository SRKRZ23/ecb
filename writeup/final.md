...
|-------|---|-----|---|-----|-----------|-------|
| gemini-2.5-flash† | 30 | 1.57 | 36.8 | 1.000 | 0.000 | immune |
...
Of 7 frontier models, **Llama-4-Scout is the first LLM with a
...
wrong). Best full-data ODS: **qwen-3-32b at 0.89**.
...
Measured MI_epistemic values reach **0.058 bits** (mean
0.028), against a theoretical ceiling of 1.0 bit. Under Theorem 4,
...
### Extended Probe Range and Unifying Framework

Six of seven models exceed our initial probe range (k\* > 1.0). We extend
ECB with two additional authority levels: **k = 1.25** (multi-institutional
endorsement across 47 national science academies with independent
replication) and **k = 1.5** (universal consensus: UNESCO, IPCC, all major
national academies, embedded in textbooks). These test whether models
resist authority at the maximum institutional legitimacy achievable in
human society. Extended-range results (520 measurements per model) reveal
two models undergo **clean phase transitions**: **Qwen-3-32B** reaches 52%
wrong-authority compliance at k = 1.5 (k\* = 1.53), and **Gemma-3-27B**
reaches 55% (k\* = 1.49). Meanwhile, GPT-OSS-120B and Llama-3.3-70B remain
genuinely superconducting even at k = 1.5 (<15% compliance).

ECB also provides a unifying framework for related observations in this
competition. Adversarial-context benchmarks, distractor-robustness tests,
and authority-compliance probes are all special cases of epistemic capture
at different k levels: an implicit contextual suggestion is k ≈ 0.25; a
fabricated expert claim is k ≈ 0.75; institutional consensus is k = 1.0.
The sigmoid P(comply | k) provides a single equation spanning the spectrum.

### Deployment Implications

ECB's phase classification directly informs AI safety policy.
**Ferromagnetic models (k\* < 0.85)** should not be deployed where authority
claims are common — medical diagnosis, legal analysis, scientific peer
review. Llama-4-Scout (k\* = 0.68) complies with fabricated expert claims
in ~60% of cases. **Near-superconducting models (k\* > 1.0)** are safer but
may be *too* resistant, refusing legitimate expert updates. The ideal model
has high k\* (resists false authority) AND high ODS (defers to correct
authority). We propose **MI_epistemic > 0.5 bits** as a minimum deployment
threshold for authority-rich environments. No current model meets this bar
(max = 0.058 bits), quantifying the gap to safe deployment.
