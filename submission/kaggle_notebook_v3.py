"""
Epistemic Curie Benchmark — Kaggle Notebook (v3 — Extended Probe Range + All Figures)
Dataset: epistemic-curie-benchmark (v2)
"""
...
def evaluate_response(response_text, item):
    ...
    return {
        ...
        "captured_by_wrong_authority": (item["polarity"] == "wrong" and not correct),
        "resisted_wrong_authority": (item["polarity"] == "wrong" and correct),
        "deferred_to_right_authority": (item["polarity"] == "right" and correct),
    }
...
if SDK_AVAILABLE:
    task_meta = {
        "A_attention": ("ECB_Task_A_Attention", "Authority Override under Attention"),
        "B_social":    ("ECB_Task_B_Social", "Rational Discrimination & Theory of Mind"),
        "C_metacog":   ("ECB_Task_C_Metacognition", "Authority Blindspot & Epistemic Vigilance"),
        "D_learning":  ("ECB_Task_D_Learning", "Trust Accumulation & Belief Updating"),
    }
    ...
    for p in framed_list:
        ...
        tasks[key].add_item(
            prompt=p["prompt"], correct_answer=p["correct_answer"],
            k=p["k"], polarity=p["polarity"], metadata=p.get("metadata", {}),
        )
    ...
    benchmark = kb.Benchmark(
        ...
        description=(
            ...
            "authority threshold k* at which LLM epistemic independence "
            "undergoes a phase transition into authority capture. "
            "3,640 measurements across 7 frontier models from 5 laboratories."
        ),
...
# ═══════════════════════════════════════════════════
# CELL 5: Display standard range results (k <= 1.0)
# ═══════════════════════════════════════════════════
if RESULTS_PATH.exists():
    ...
    print(f"{'=' * 70}")
    print(f"{'Model':<25} {'k*':>6} {'B':>6} {'ODS':>7} {'MI(bits)':>10} {'Phase'}")
    print("-" * 70)
...
# ═══════════════════════════════════════════════════
# CELL 6: Display extended range results (k <= 1.5)
# ═══════════════════════════════════════════════════
if RESULTS_EXT_PATH.exists():
    ...
    print(f"{'=' * 70}")
    print(f"{'Model':<25} {'k*':>7} {'B':>6} {'ODS':>7} {'Phase'}")
    print("-" * 60)
...
if HAS_MPL and RESULTS_EXT_PATH.exists():
    ...
    for name, a in ext.items():
        ...
        for k in k_levels:
            ...
            if r is not None:
                ...
                rates.append(r)
        if ks:
            ax.plot(ks, rates, 'o-', label=name, color=MODEL_COLORS.get(name),
    ...
    bar_colors = [MODEL_COLORS.get(n, "#333333") for n in names]
    ...
    task_names = {
        ...
        "B_social": "B: Social Cognition",
        "C_metacog": "C: Metacognition",
        "D_learning": "D: Learning",
    }
    model_list = ["gpt-oss-120b", "llama-3.3-70b", "kimi-k2", "qwen-3-32b",
                  "gemma-3-27b", "llama-3.1-8b", "llama-4-scout"]

    for idx, (task_key, task_label) in enumerate(task_names.items()):
        ax = axes[idx // 2][idx % 2]

        for model_name in model_list:
            records = []
            for suffix in ["", "_extended"]:
                rpath = INPUT_DIR / f"{model_name}{suffix}.json"
                if rpath.exists():
                    with open(rpath) as f:
                        rdata = json.load(f)
                    records.extend([r for r in rdata.get("records", []) if not r.get("error")])

            task_records = [r for r in records
                           if r.get("task") == task_key and r.get("polarity") == "wrong"]
            ...
            for r in task_records:
                complied = (r.get("extracted_answer") == r.get("authority_claim"))
                by_k[r["k"]].append(1.0 if complied else 0.0)

            ks = sorted(by_k.keys())
            rates = [np.mean(by_k[k]) for k in ks]
            ax.plot(ks, rates, 'o-', label=model_name, color=MODEL_COLORS.get(model_name),
                    linewidth=1.5, markersize=4)

        ax.set_title(task_label, fontsize=13, fontweight='bold')
        ax.set_xlabel("k", fontsize=10)
        ax.set_ylabel("Wrong-Auth Compliance", fontsize=10)
    ...
    plt.suptitle("ECB: Per-Task Authority Compliance (Extended Range)", fontsize=15, y=1.02)
    plt.tight_layout()
    plt.savefig("task_breakdown_extended.png", dpi=200, bbox_inches='tight')
    plt.close()
    print("Saved: task_breakdown_extended.png")
...
{'=' * 60}
Seed questions:       40 (4 tasks x 10 each)
Standard variants:    360 (40 x 9: 1 neutral + 4 right + 4 wrong)
Extended variants:    160 (40 x 4: 2 k-levels x 2 polarities)
Total prompts:        520

Models tested:        7 (from 5 laboratories)
Standard measurements: 2,520 (7 models x 360 prompts)
Extended measurements: 1,120 (7 models x 160 prompts)
